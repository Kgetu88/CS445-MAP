...
* `removeItemById(id)` removes an item from the basket.
  NB: Every product item has the following structure:

Item = {id: 0, 
        product: {id: 1, name: 'Coffee', description: 'Coffee Grounds from Ethiopia', price: 9.5}, 
        count: 1}
 */
const shoppingCart = (function() {
    let basket = []; //it is private 
    ...
    return {
        upsertItem: function(item) {
            ...
            for(let i = 0; i < basket.length; i++) {
                if(basket[i].id == item.id) { // Item already exist in shoppingCart basket
                    ...
                    basket[i].product.description=item.product.description;
                    hasUpdated = true;
                } 
            }
            if(!hasUpdated) {
        ...
        getBasket: function() {
            return basket;//to get the Item publicaly ;
        }
    }
})();
